################################################################################
# Layer 2: Platform - Terraform Backend Configuration
#
# This file configures remote state management for Layer 2 infrastructure.
...
#
# State file path: s3://<bucket>/layer2-platform/terraform.tfstate
# Lock table: <dynamodb_table> with key: layer2-platform/terraform.tfstate
################################################################################

terraform {
  backend "s3" {
    key = "layer2-platform/terraform.tfstate"
  }
}
