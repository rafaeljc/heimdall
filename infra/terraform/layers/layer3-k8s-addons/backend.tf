################################################################################
# Layer 3: K8s Add-ons - Remote State Configuration
#
# This file configures remote state management for Layer 3 infrastructure.
...
#
# State file path: s3://<bucket>/layer3-k8s-addons/terraform.tfstate
# Lock table: <dynamodb_table> with key: layer3-k8s-addons/terraform.tfstate
################################################################################

terraform {
  backend "s3" {
    key = "layer3-k8s-addons/terraform.tfstate"
  }
}
